...
Everything here talks to a dedicated ``finos_test`` Postgres database on the
...
``AsyncSessionLocal``/``lifespan`` (which would touch the real ``finos``
...
def _test_database() -> Generator[None, None, None]:
    """Create finos_test from scratch and run every Alembic migration
    against it. Connects to Postgres' always-present `postgres` maintenance
    database to issue CREATE/DROP DATABASE — the real `finos` database is
    ...
    env = {**os.environ, "FINOS_POSTGRES_DB": TEST_DB_NAME}
