...
async def build_backup(session: AsyncSession) -> BackupPayload:
    ...
    return BackupPayload(
        finos_backup_version=BACKUP_FORMAT_VERSION,
...
async def restore_backup(session: AsyncSession, payload: BackupPayload) -> None:
    if payload.finos_backup_version != BACKUP_FORMAT_VERSION:
        raise HTTPException(
            400,
            f"Unsupported backup version {payload.finos_backup_version} "
            f"(this FinOS version supports {BACKUP_FORMAT_VERSION})",
        )
