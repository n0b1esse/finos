...
class BackupPayload(BaseModel):
    """A full, portable snapshot of every table. `finos_backup_version` is
    checked on import so an incompatible/future file is rejected cleanly
    instead of half-applied."""

    model_config = ConfigDict(extra="ignore")

    finos_backup_version: int

    @model_validator(mode="before")
    @classmethod
    def _compat_previous_brand_version(cls, data: Any) -> Any:
        # Backups exported before the rename carry `aurum_backup_version`
        # instead of `finos_backup_version` — accept them so users don't
        # lose their restore path on upgrade.
        if isinstance(data, dict) and "finos_backup_version" not in data and "aurum_backup_version" in data:
            data = {**data, "finos_backup_version": data["aurum_backup_version"]}
        return data
