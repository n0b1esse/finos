...
FinOS exposes the same REST API its own frontend uses. Every action available in the UI — adding a
transaction, creating an account, tagging an expense, importing a CSV statement, tracking an asset,
setting a budget — can be done directly over HTTP. This makes it possible to script FinOS, feed it
...
FinOS ships as three containers (Postgres, FastAPI backend, nginx-served frontend). The frontend
container reverse-proxies `/api/*` straight through to the backend, so **the API and the web UI share
the same host and port** — whatever you set `FINOS_WEB_PORT` to in `.env` (default `3000`):
...
- **Money fields** (`amount`, `monthly_limit`, `value`, ...): decimal numbers as JSON numbers or
  strings, up to 14 digits with 2 decimal places. FinOS is single-currency per account/asset — there
...
Requires `FINOS_COINGECKO_API_KEY` in `.env` (a free Demo key, no card required). Endpoints that need
...
Read-only aggregation endpoints — the numbers behind FinOS's charts. Useful for pulling summary data
...
```bash
# Export
curl -u user:pass http://localhost:3000/api/backup/export -o finos-backup.json
...
The payload includes an `finos_backup_version` field checked on import — a file from an incompatible
...
to IDs (via `GET /accounts` / `GET /categories`), and, since FinOS itself never converts currency (see
...
For self-hosting, environment variables, and running FinOS itself, see [README.md](README.md).
