...
const ACK_KEY = "finos:noAuthAcknowledged";
// Previous brand key — checked once as fallback, then migrated.
const LEGACY_ACK_KEY = "aurum:noAuthAcknowledged";

/** Shown by LoginGate once (ever, per browser) when this instance has no
 * FINOS_BASIC_AUTH_USER/PASSWORD configured (see frontend/docker-entrypoint.d/
...
export function NoAuthModal() {
  ...
  const [acknowledged, setAcknowledged] = useState(() => {
    try {
      ...
      // One-time migration from the previous brand key.
      if (localStorage.getItem(LEGACY_ACK_KEY) === "1") {
