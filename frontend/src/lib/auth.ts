 ...
 */
const SESSION_KEY = "finos:basicAuth";
...
// clear it.
const LEGACY_REMEMBER_KEY = "finos:basicAuth:remember";
// Previous brand plaintext key — deleted on boot like the current one,
// so an upgrade never leaves the password on disk.
const PREV_BRAND_LEGACY_REMEMBER_KEY = "aurum:basicAuth:remember";
...
function forgetLegacyRememberedCredentials(): void {
  try {
    ...
    localStorage.removeItem(PREV_BRAND_LEGACY_REMEMBER_KEY);
  } catch {
...
const PROBE_HEADER = `Basic ${btoa("__finos_probe__:__finos_probe__")}`;
