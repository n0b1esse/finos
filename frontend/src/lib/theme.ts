...
// Previous brand key — read once as fallback, then migrated to the new key.
const LEGACY_STORAGE_KEY = "aurum:theme";
...
function readInitialTheme(): Theme {
  ...
  if (stored === "light" || stored === "dark" || stored === "system") {
    ...
    try {
      ...
      // storage unavailable — will retry next load, harmless
    }
    return stored;
  }
  return "system";
